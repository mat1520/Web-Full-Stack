...
(() => {
  ...
  const dom = {
    ...
    commentAuthPrompt: $("comment-auth-prompt"),
    btnPromptLogin: $("btn-prompt-login"),
    ...
    fileNameVideo: $("file-name-video"),
    fileNameThumb: $("file-name-thumb"),
    dropZone: $("drop-zone"),
  ...
  const request = async (endpoint, options = {}) => {
    ...
    if (!res.ok) {
      ...
      try {
        err = (await res.json()).detail || `Error ${res.status}`;
      } catch (e) {}
      throw new Error(err);
    }
  ...
  const api = {
    ...
    authRequest: (ep, data) =>
      request(`/auth/${ep}`, { method: "POST", body: data, noAuth: true }),
      ...
      new Promise((resolve, reject) => {
        ...
        xhr.onload = () =>
          xhr.status === 201
            ...
            : reject(
                new Error(
                  JSON.parse(xhr.responseText)?.detail || "Upload error",
                ),
              );
  ...
  const updateOwnershipUI = () => {
    dom.btnDeleteVideo.classList.toggle(
      "view--hidden",
      !(
        state.currentVideo &&
        state.user &&
        state.currentVideo.owner_id === state.user.id
      ),
    );
  };
  ...
  const handleUploadSubmit = async (e) => {
    ...
    const desc = dom.uploadDesc.value.trim();
    ...
    if (!title || !category || !videoFile || !thumbFile) {
      dom.uploadError.textContent =
        "Por favor, completa todos los campos obligatorios y selecciona ambos archivos.";
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      const saved = await api.postComment({
        video_id: state.currentVideo.id,
        content,
      });
      state.currentVideo.comments.push(saved);
      dom.commentsList
        .querySelector(".comment--optimistic")
        ?.classList.remove("comment--optimistic");
      ...
      dom.commentForm.querySelector(".comment-form__submit").disabled = false;
    }
  };
  ...
  const updateActiveCategory = (cat) => {
    ...
    dom.categoryList
      .querySelectorAll(".category-filter__btn")
      .forEach((btn) => {
        const match =
          (btn.dataset.category || null) === (cat || null) ||
  ...
  const filterByCategory = (cat) => {
    ...
    const filtered = cat
      ? state.videos.filter((v) => v.category === cat)
      : state.videos;
    clear(dom.videoGrid);
    if (!filtered.length)
      return renderState(dom.videoGrid, "No hay videos en esta categoría");
  ...
  const showDetailView = async (id) => {
    ...
    try {
      ...
      dom.mainVideo
        .play()
        .then(() => (dom.mainVideo.muted = false))
        .catch(() => {});
    ...
    try {
      if (!Object.keys(state.recommendations).length)
        state.recommendations = await api.getRandomPicks();
      const all = Object.values(state.recommendations)
        .flat()
        .filter((v) => v.id !== id);
  ...
  const initDragAndDrop = () => {
    ...
    ["dragenter", "dragover", "dragleave", "drop"].forEach((e) =>
      dom.dropZone.addEventListener(e, pd),
    );
    ["dragenter", "dragover"].forEach((e) =>
      dom.dropZone.addEventListener(e, () =>
    ...
    dom.dropZone.addEventListener("drop", (e) => {
      Array.from(e.dataTransfer.files).forEach((f) => {
        ...
        if (f.type.startsWith("video/")) {
          dom.uploadVideo.files = dt.files;
          dom.fileNameVideo.textContent = f.name;
        } else if (f.type.startsWith("image/")) {
          dom.uploadThumb.files = dt.files;
          dom.fileNameThumb.textContent = f.name;
        }
      });
    });
  };
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    dom.brandLink.onclick = (e) => {
      e.preventDefault();
      navigateTo("/");
    };
    dom.commentForm.onsubmit = handleCommentSubmit;
    dom.btnPromptLogin.onclick = dom.btnLogin.onclick = () =>
      openAuthModal("login");
    ...
    dom.uploadVideo.onchange = (e) =>
      (dom.fileNameVideo.textContent =
        e.target.files[0]?.name || "No seleccionado");
    dom.uploadThumb.onchange = (e) =>
      (dom.fileNameThumb.textContent =
        e.target.files[0]?.name || "No seleccionado");
    dom.btnDeleteVideo.onclick = () =>
      state.currentVideo && dom.modalConfirm.showModal();
